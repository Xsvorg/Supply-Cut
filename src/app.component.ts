...
import { TestTerminalComponent } from './components/test-terminal.component';

@Component({
  selector: 'app-root',
  standalone: true,
  imports: [CommonModule, CardComponent, GridComponent, TestTerminalComponent],
  template: `
    <div class="h-screen w-screen bg-[#0f1115] text-slate-200 crt overflow-hidden font-mono flex flex-col relative">
      
      <!-- DYNAMIC BACKGROUND -->
      <div [class]="'absolute inset-0 -z-10 transition-all duration-1000 ease-in-out bg-cover bg-center ' + bgThemeClass()">
           <div class="absolute inset-0 bg-black/50 mix-blend-multiply"></div> <!-- Vignette overlay -->
           <div class="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/stardust.png')] opacity-20"></div>
           
           @if(game.currentBattle()?.backgroundTheme === 'desert') {
               <div class="absolute inset-0 bg-gradient-to-t from-orange-900/30 to-blue-900/10"></div>
           }
           @if(game.currentBattle()?.backgroundTheme === 'snow') {
               <div class="absolute inset-0 bg-gradient-to-b from-slate-200/10 to-blue-900/30"></div>
           }
      </div>
        ...
        <div class="flex-1 w-full h-full flex flex-col lg:flex-row gap-4 p-4 relative z-10 transition-all duration-300">
            ...
            <div class="flex items-center justify-between bg-slate-900/90 p-4 border-b-2 border-slate-700 rounded-lg backdrop-blur shadow-lg shrink-0">
                ...
                <div class="flex flex-col">
                     ...
                     <span class="text-xl font-mono text-amber-500 tracking-widest uppercase font-bold flex items-center gap-2">
                        <span class="text-xs bg-amber-900/50 text-amber-500 px-1 rounded border border-amber-800">{{ game.currentBattle().year }}</span>
                        {{ game.currentBattle().name }}
                     </span>
                </div>
                <div class="h-8 w-px bg-slate-600"></div>
                <div>SECURED: <span class="text-emerald-400 font-bold text-2xl ml-2">{{ game.routesCutCount() }}/{{ game.currentBattle().routesToCut }}</span></div>
                </div>
                <div class="flex gap-4 items-center font-mono">
                <button (click)="game.togglePause()" class="px-4 py-2 border border-slate-600 bg-slate-800 text-slate-300 rounded hover:bg-slate-700 transition-colors text-xs font-bold tracking-wider uppercase min-w-[80px]">
                ...
                <button (click)="sound.toggleMute()" class="p-3 bg-slate-800 hover:bg-slate-700 rounded-full border border-slate-600 transition-colors">🔊</button>
                </div>
            </div>
            ...
            <div class="grid gap-4 shrink-0 h-16" [style.grid-template-columns]="'repeat(' + game.routes().length + ', minmax(0, 1fr))'">
                ...
                <div class="bg-slate-900/80 p-2 px-4 rounded border border-slate-700 flex flex-col justify-center relative overflow-hidden transition-all duration-500 group hover:border-slate-500 shadow-lg"
                    ...
                    @if (route.isCut) { <span class="text-[10px] text-black font-bold bg-emerald-400 px-2 py-0.5 rounded shadow-lg animate-bounce">SECURE</span> } 
                    @else { <span class="text-[10px] text-emerald-500 font-bold border border-emerald-500/30 px-2 rounded">ACTIVE</span> }
                    </div>
            ...
            <div class="bg-slate-800/95 border-4 border-slate-700 rounded-xl p-4 flex flex-col items-center relative overflow-hidden shadow-2xl transition-all shrink-0 backdrop-blur">
                ...
                <div class="text-slate-400 text-sm font-mono mb-2 z-10 tracking-[0.2em] uppercase border-b border-slate-600 pb-1 w-full text-center flex justify-between">
                    <span>Intercepted Signal</span> <span class="text-red-500 animate-pulse">● REC</span>
                </div>
                ...
                <div class="w-full mt-4 z-10 relative">
                    <div class="flex justify-between text-xs text-slate-400 mb-2 font-bold relative">
                        <span>ENEMY TRACE SIGNAL</span>
                        <span>{{ game.threatLevel() | number:'1.0-0' }}%</span>

                        <!-- THREAT CHANGE INDICATOR -->
                        @if(game.threatChange(); as change) {
                            <div class="absolute right-0 -top-6 text-lg font-black animate-ping-once"
                                 [class.text-red-500]="change.amount > 0"
                                 [class.text-emerald-500]="change.amount <= 0">
                                {{ change.amount > 0 ? '+' : '' }}{{ change.amount | number:'1.0-0' }}% {{change.reason}}
                            </div>
                        }
                    </div>
                    ...
                    <div class="h-full transition-all duration-300 ease-out"
                        ...
                        [class.from-red-900]="game.threatLevel() > 80" [class.to-red-600]="game.threatLevel() > 80"
                        [class.from-amber-700]="game.threatLevel() <= 80" [class.to-amber-500]="game.threatLevel() <= 80"
                    ...
                    @if (game.threatLevel() > 80) { <div class="absolute inset-0 bg-red-500/30 animate-ping"></div> }
                    </div>
                    <div class="text-xs text-red-400 mt-2 text-center animate-pulse font-black tracking-widest h-4" [class.invisible]="game.threatLevel() < 80">⚠️ DETECTION IMMINENT ⚠️</div>
                </div>
                } @else { <div class="text-6xl text-slate-600 font-stencil z-10 py-8">--</div> }
            </div>

            <div class="relative bg-black/80 rounded-lg border border-slate-700 flex-[0.5] overflow-hidden flex flex-col font-mono text-sm shadow-inner min-h-[150px] backdrop-blur-sm">
                ...
                <div class="text-slate-500 bg-slate-900/80 px-4 py-2 text-xs font-bold border-b border-slate-700 uppercase tracking-wider z-20 flex justify-between shrink-0">
                    <span>Comms Log</span> <span class="text-emerald-500 animate-pulse">● LIVE</span>
                </div>
                <div class="flex-1 overflow-y-auto p-4 flex flex-col gap-2 pt-4">
                    @for (log of game.logs(); track log.id) {
                    <div class="animate-in slide-in-from-left-2 fade-in duration-300 flex gap-3 items-start leading-tight border-b border-slate-800/50 pb-1">
                        ...
                        <span class="font-bold tracking-wide"
                            ...
                            [class.text-slate-300]="log.type==='INFO'">{{ log.text }}</span>
                    </div>
                    }
                </div>
            </div>

            <div class="flex-1 flex flex-col justify-end relative bg-slate-900/80 rounded-xl p-3 border border-slate-600 shadow-2xl min-h-[300px]">
                <div class="text-xs text-slate-500 mb-3 text-center uppercase tracking-[0.3em] font-bold border-b border-slate-700 pb-2">Field Orders</div>
                <div class="grid grid-cols-3 gap-3 mb-4 flex-1 items-end">
                    @for (card of game.hand(); track card.id; let i = $index) {
                    <div class="flex justify-center h-full animate-in slide-in-from-bottom-4 duration-500" [style.animation-delay]="(i * 100) + 'ms'">
                        <app-card class="w-full h-full" [card]="card" [targetValue]="game.currentCall()" (cardClicked)="game.playCard($event)"/>
                    </div>
                    }
                </div>
                <button class="w-full bg-gradient-to-r from-slate-800 to-slate-700 hover:from-slate-700 hover:to-slate-600 border-2 border-slate-500 text-slate-200 font-bold py-4 px-6 rounded-lg uppercase tracking-widest transition-all text-sm flex items-center justify-between group shadow-xl active:scale-[0.98] shrink-0"
                    (click)="game.recycleHand()" [disabled]="game.resources().supplies < 1">
                ...
                <button class="w-full mt-3 bg-red-900/90 hover:bg-red-800 border-2 border-red-500 text-white font-bold py-3 px-4 rounded-lg uppercase tracking-widest transition-all text-xs flex items-center justify-between animate-pulse shadow-[0_0_15px_rgba(239,68,68,0.5)] shrink-0"
                    (click)="game.emergencyResupply()">
                    <span class="flex items-center gap-2 text-lg">⚠️ <span class="text-sm">EMERGENCY DROP</span></span>
                    <div class="flex flex-col items-end text-[10px]"><span>+3 SUPPLY</span><span class="text-red-300">+25% THREAT</span></div>
                </button>
                }
            </div>
            </div>
      ...
      <!-- OVERLAYS: MENU, BRIEFING, MODALS etc. -->
        ...
        <div class="absolute inset-0 bg-black flex flex-col items-center justify-center z-50">
          ...
          <div class="relative z-10 text-center space-y-8 animate-in fade-in duration-1000 zoom-in-90">
            ...
            <div class="flex gap-8 mt-12 justify-center">
              <div class="w-80 h-96 bg-slate-900/50 border-4 border-slate-700 hover:border-emerald-500 rounded-xl p-6 flex flex-col items-center justify-center gap-4 cursor-pointer transition-all hover:scale-105 hover:bg-slate-900" (click)="game.selectFaction('ALLIES')">
              ...
              <div class="w-80 h-96 bg-slate-900/50 border-4 border-slate-700 hover:border-red-500 rounded-xl p-6 flex flex-col items-center justify-center gap-4 cursor-pointer transition-all hover:scale-105 hover:bg-slate-900" (click)="game.selectFaction('AXIS')">
                <div class="text-6xl">🦅</div>
            ...
            <button (click)="game.startDiagnostics()" class="mt-8 text-xs text-slate-600 hover:text-green-500 font-mono uppercase tracking-widest border-b border-transparent hover:border-green-500 transition-colors">
              [ Field Manual Diagnostics ]
            </button>
          </div>
        </div>
      }
      @if (game.gameState() === 'DIAGNOSTICS') { <app-test-terminal /> }
      ...
      @if (game.gameState() === 'BRIEFING') {
        <div class="absolute inset-0 bg-black/90 backdrop-blur-md flex items-center justify-center z-50 px-4">
          <div class="bg-slate-900 border-2 border-slate-600 p-10 max-w-2xl w-full shadow-2xl rounded-xl text-center relative overflow-hidden">
            ...
            <p class="text-xl text-slate-300 font-mono mb-6 leading-relaxed border-y border-slate-700 py-6">"{{ game.currentBattle().description }}"</p>
            ...
            <button (click)="game.startBattle()" class="w-full bg-amber-600 hover:bg-amber-500 text-black font-black py-5 px-6 rounded-lg uppercase tracking-[0.2em] transition-all text-2xl shadow-[0_0_20px_rgba(217,119,6,0.5)] hover:shadow-[0_0_30px_rgba(217,119,6,0.8)]">
              Deploy
            </button>
      ...
      @if (['VICTORY', 'DEFEAT', 'WAR_WON'].includes(game.gameState())) {
        <div class="absolute inset-0 bg-black/90 backdrop-blur-md flex items-center justify-center z-50 animate-in fade-in duration-500 px-4">
          <div class="text-center max-w-xl border-4 border-slate-700 bg-slate-900 p-10 rounded-2xl shadow-2xl relative">
            <h1 class="text-7xl font-black font-stencil mb-6 drop-shadow-lg" [class.text-emerald-500]="game.gameState() === 'VICTORY' || game.gameState() === 'WAR_WON'" [class.text-red-500]="game.gameState() === 'DEFEAT'">
            ...
            @if (game.gameState() === 'WAR_WON') {
              ...
              <button (click)="game.initMenu()" class="bg-emerald-700 hover:bg-emerald-600 text-white font-bold py-5 px-10 rounded-lg border-2 border-emerald-500 uppercase w-full text-xl tracking-widest hover:scale-105 transition-transform">
                Return to Menu
              </button>
            } @else {
              ...
              @if (game.gameState() === 'VICTORY') {
                <button (click)="game.nextLevel()" class="bg-emerald-700 hover:bg-emerald-600 text-white font-bold py-5 px-10 rounded-lg border-2 border-emerald-500 uppercase w-full text-xl tracking-widest hover:scale-105 transition-transform">
                  Next Battle
                </button>
              }
              @if (game.gameState() === 'DEFEAT') {
                <button (click)="game.restartLevel()" class="bg-slate-700 hover:bg-slate-600 text-white font-bold py-5 px-10 rounded-lg border-2 border-slate-500 uppercase w-full text-xl tracking-widest hover:scale-105 transition-transform">
                  Retry Mission
                </button>
              }
...
export class AppComponent {
  ...
  readonly bgThemeClass = computed(() => {
     const theme = this.game.currentBattle()?.backgroundTheme;
     switch(theme) {
         case 'desert': return 'bg-amber-900/30 bg-[url("https://www.transparenttextures.com/patterns/sandpaper.png")]';
         case 'snow': return 'bg-slate-200/20 bg-[url("https://www.transparenttextures.com/patterns/snow.png")]';
         case 'coastal': return 'bg-cyan-900/40 bg-[url("https://www.transparenttextures.com/patterns/water.png")]';
         case 'forest': return 'bg-emerald-900/30 bg-[url("https://www.transparenttextures.com/patterns/wood.png")]';
         case 'urban': return 'bg-slate-800/80 bg-[url("https://www.transparenttextures.com/patterns/concrete-wall.png")]';
         case 'plains': return 'bg-yellow-900/20 bg-[url("https://www.transparenttextures.com/patterns/grass.png")]';
